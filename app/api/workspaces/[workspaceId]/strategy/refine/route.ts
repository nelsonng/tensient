...
import { refineStrategy } from "@/lib/services/genesis-setup";
...
export async function POST(
  ...
  const { canonId, feedback } = await request.json();

  if (!canonId) {
    return NextResponse.json(
      { error: "canonId is required" },
      { status: 400 }
    );
  }
  ...
  try {
    const { result, usage } = await refineStrategy(
      workspaceId,
      canonId,
      feedback.trim()
    );
